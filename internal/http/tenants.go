...
func (h *TenantsHandler) handleUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if input.Status != "" {
		if !store.IsValidTenantStatus(input.Status) {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": i18n.T(locale, i18n.MsgInvalidRequest, "invalid tenant status")})
			return
		}
...
func (h *TenantsHandler) handleDelete(w http.ResponseWriter, r *http.Request) {
	...
	if id == store.MasterTenantID {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "master tenant cannot be deleted"})
		return
	}
